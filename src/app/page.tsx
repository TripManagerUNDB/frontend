    ...
    <div className="min-h-screen">
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-12 w-full relative z-10">
          <div className="max-w-[720px]">
            <div className={`inline-flex items-center gap-2 mb-7 px-3.5 py-1.5 border border-gold/25 rounded-full bg-gold/5 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-7'} delay-100`}>
              <span className="w-1.5 h-1.5 rounded-full bg-gold animate-pulse-custom" />
              <span className="text-[12px] tracking-widest text-gold uppercase font-medium">Planejamento com IA</span>
            </div>

            <h1 className={`font-display text-[clamp(52px,6vw,88px)] font-medium leading-[1.08] tracking-tight text-foreground mb-7 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-7'} delay-200`}>
              Sua próxima viagem,<br />
              <em className="italic text-gold not-italic">planejada em segundos.</em>
            </h1>

            <p className={`text-lg text-muted leading-relaxed mb-11 max-w-[520px] transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-7'} delay-350`}>
              TripManager usa inteligência artificial para criar roteiros personalizados, otimizar rotas e estimar custos — tudo em tempo real, para qualquer destino do mundo.
            </p>
            ...
            <div className={`flex gap-4 items-center flex-wrap transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-7'} delay-500`}>
              ...
              <button className="btn-ghost text-[15px]">Ver como funciona</button>
            </div>

            <div className={`flex gap-8 mt-14 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-7'} delay-650`}>
              {[['12k+','Roteiros criados'], ['98%','Satisfação'], ['180+','Países']].map(([n, l]) => (
                <div key={l}>
                  <div className="font-mono text-[26px] text-gold font-medium">{n}</div>
        ...
        {/* Hero mockup preview */}
        <div className={`hidden lg:block absolute right-12 top-1/2 -translate-y-1/2 w-[420px] transition-all duration-1000 ${visible ? 'opacity-100 translate-y-[-50%]' : 'opacity-0 translate-y-[-42%]'} delay-400`}>
          <HeroMockup />
        </div>
      </section>
      ...
      <section className="py-24 px-12 max-w-[1200px] mx-auto">
        <div className="text-center mb-16">
          <div className="text-[11px] tracking-[0.14em] text-gold uppercase font-semibold mb-4">Como funciona</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="card p-9 relative overflow-hidden">
              <div className="font-mono text-[64px] font-medium text-gold/10 absolute top-3 right-5 leading-none select-none">{s.num}</div>
              <div className="w-10 h-10 rounded-full bg-gold-dim border border-gold/30 flex items-center justify-center mb-6">
                <span className="font-mono text-[13px] text-gold font-medium">{s.num}</span>
              </div>
              <h3 className="font-display text-[22px] font-medium mb-3 text-foreground">{s.title}</h3>
      ...
      <section className="py-20 px-12 bg-white/5 border-y border-border-light">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-14">
            <div className="text-[11px] tracking-[0.14em] text-gold uppercase font-semibold mb-4">Depoimentos</div>
            <h2 className="font-display text-[clamp(28px,3vw,40px)] font-medium text-foreground">O que dizem os viajantes</h2>
          </div>
            ...
            {[
              ...
              <div key={i} className="card p-7 relative">
                <div className="relative overflow-hidden">
                  ...
                  <p className="text-[14px] text-foreground leading-relaxed mb-6 relative italic">{t.text}</p>
                </div>
                <div className="flex justify-between items-end">
                  ...
                  <div className="text-[11px] font-mono text-gold bg-gold-dim px-2.5 py-1 rounded-full border border-gold/20">{t.dest}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="border-t border-border-light py-8 px-12 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-7">
          {['Sobre', 'Privacidade', 'Termos', 'Contato'].map(l => (
            <span key={l} className="text-[13px] text-dim cursor-pointer transition-colors hover:text-muted">{l}</span>
          ))}
        </div>
    ...
    <div className="relative">
      ...
      <div className="bg-[#1a2d3a] border border-white/5 rounded-xl overflow-hidden shadow-[0_40px_80px_rgba(0,0,0,0.7)] relative">
        ...
        <div className="px-[18px] py-[14px] border-b border-white/5 flex justify-between items-center">
          ...
          <div className="bg-gold-dim border border-gold/30 rounded px-2.5 py-1 text-[11px] text-gold font-mono">R$ 8.400</div>
        </div>
        {/* Map placeholder */}
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 420 160">
            ...
            {[[80,90],[200,75],[330,65]].map(([x,y],i) => (
              <g key={i}>
                ...
                <text x={x} y={y+4} textAnchor="middle" fill="#12212E" fontSize="8" fontFamily="DM Mono" fontWeight="600">{i+1}</text>
              </g>
            ))}
          </svg>
        </div>
        {/* Day list */}
        {[['09:00', 'Torre Eiffel', '🗼', 'R$ 120'],['12:00', 'Café de Flore', '☕', 'R$ 80'],['15:00', 'Musée du Louvre', '🎨', 'R$ 90']].map(([t, name, ic, cost]) => (
