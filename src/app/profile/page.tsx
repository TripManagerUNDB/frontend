        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 28 }}>
              {[
                { n: trips.length, l: 'Viagens' },
                { n: totalDays, l: 'Dias viajados' },
              ].map(({ n, l }) => (
          ...
          <div style={{ display: 'flex', gap: 0, marginTop: 32, borderBottom: '1px solid var(--border-light)', marginLeft: -48, paddingLeft: 48 }}>
            {(['viagens', 'plano'] as const).map(tab => (
              <button
                ...
                style={{ padding: '10px 20px', fontSize: 13, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', color: activeTab === tab ? 'var(--gold)' : 'var(--text-muted)', borderBottom: activeTab === tab ? '2px solid var(--gold)' : '2px solid transparent', marginBottom: -1, textTransform: 'capitalize' }}
              >
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
              {FILTERS.map(f => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  style={{ padding: '6px 14px', borderRadius: 100, fontSize: 12, cursor: 'pointer', border: filter === f ? '1px solid var(--gold)' : '1px solid var(--border)', background: filter === f ? 'var(--gold-dim)' : 'transparent', color: filter === f ? 'var(--gold)' : 'var(--text-muted)' }}
                >
                  {FILTER_LABELS[f]}
                </button>
              ))}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
                {trips.map(trip => (
                  <div
                    ...
                    style={{ background: '#1a2d3a', border: '1px solid var(--border-light)', borderRadius: 10, overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(200,169,110,0.3)'; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border-light)'; }}
                  >
                    <div style={{ height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, ${trip.color}22, ${trip.color}11)`, fontSize: 36 }}>
                      {trip.emoji || '✈️'}
                    </div>
                    <div style={{ padding: '14px 16px' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--ff-mono)' }}>
                          {trip.days} dias
                        </span>
                      </div>
                      <button
